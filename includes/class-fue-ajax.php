...
class FUE_AJAX {
    ...
    public static function send_manual_emails() {
        ...
        if ( $cmd == 'start' ) {
            ...
            if ( $status == 'completed' ) {
                ...
                set_transient( 'fue_manual_email_'. $key, $data, 86400, 100 );
            }
            ...
            if ( $cmd == 'start' ) {
                ...
                foreach ( $email_id as $id ) {
                    ...
                    if ( !empty( $orders ) ) {
                        ...
                        self::set_transient( 'fue_import_'. $id .'_'. $session, $orders, 86400 );
                    }
                ...
                $orders         = self::get_transient( 'fue_import_'. $current_email_id .'_'. $session );
                ...
                $filtered = self::get_transient( 'fue_import_filtered_'. $current_email_id .'_'. $session );
                ...
                self::delete_transient( 'fue_import_'. $current_email_id .'_'. $session );
                self::set_transient( 'fue_import_filtered_'. $current_email_id .'_'. $session, $filtered, 86400 );
                ...
                do {
                    ...
                    $orders = self::get_transient( 'fue_import_filtered_'. $id .'_'. $session );
                } while ( $orders === false );
                ...
                $results = $wc_importer->import_orders( $id, $orders );

                self::set_transient( 'fue_import_filtered_'. $id .'_'. $session, $results['orders'] );
    ...
    public static function wc_update_customer_order_total() {
        ...
        if ( $cmd == 'start' ) {
            ...
            self::set_transient( 'fue_update_'. $session, $orders, 3600 );
            ob_clean();
            // re-enable caching if it was previously enabled
            $_wp_using_ext_object_cache = $_wp_using_ext_object_cache_previous;
            self::send_response( array(
            ...
            ob_start();
            $orders = self::get_transient( 'fue_update_'. $session );
            if ( $orders === false ) {
            ...
            self::set_transient( 'fue_update_'. $session, $orders );
    ...
    public static function wc_subscriptions_update() {
        ...
        if ( $cmd == 'start' ) {
            ...
            if ( !FUE_Addon_Subscriptions::is_wcs_2() ) {
                self::set_transient( 'fue_update_'. $session, array(), 600 );
            ...
            self::set_transient( 'fue_update_'. $session, $filtered, 600 );
            ...
            $items = self::get_transient( 'fue_update_'. $session );
            ...
            self::set_transient( 'fue_update_'. $session, $items );
     ...
     */
    private static function set_transient( $name, $data, $expiration = 0, $length = 1000 ) {
        $suffix = 1;

        if ( !is_array($data) ) {
            // straight-up store the data
    ...
    private static function delete_transient( $name ) {
        global $wpdb;
        ...
        do {
            ...
            if ( $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->options} WHERE option_name = %s", $key)) == 0 ) {
                break;
            }
     ...
     * @return mixed
     */
    private static function get_transient( $name ) {
