...
class FUE_Addon_Woocommerce_Mailer {
    ...
    private function get_replacement_data( $email_data, $queue_item, $email ) {
        ...
        if ( $queue_item->is_cart ) {
            ...
            $replacements['cart_contents']          = $this->get_persistent_cart_table( $queue_item->user_id );
            $replacements['cart_total']             = $this->get_persistent_cart_total( $queue_item->user_id );
            $replacements['cart_url']               = add_query_arg( 'fue_cart_redirect', 1, get_permalink( woocommerce_get_page_id( 'cart' ) ) );
    ...
    private function get_test_replacement_data( $email_data, $email ) {
        ...
        if ( $email_type == 'storewide' ) {
            ...
            $replacements['item_categories']        = $item_cats;
            $replacements['item_category']          = $item_cats;
            $replacements['cart_contents']          = $this->get_persistent_cart_table( get_current_user_id() );
            $replacements['cart_total']             = $this->get_persistent_cart_total( get_current_user_id() );
            $replacements['cart_url']               = add_query_arg( 'fue_cart_redirect', 1, get_permalink( wc_get_page_id( 'myaccount' ) ) );
     ...
     * @return string
     */
    public function get_persistent_cart_total( $user_id ) {
        $this->init_wc_cart( $user_id );
     ...
     * @return string
     */
    public function get_persistent_cart_table( $user_id ) {
        $this->init_wc_cart( $user_id );
    ...
    private function init_wc_cart( $user_id ) {
        ...
        $cart = $this->fue_wc->get_persistent_cart( $user_id );
