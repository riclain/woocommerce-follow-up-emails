...
function fue_get_login_url( $redirect = '' ) {
    if ( Follow_Up_Emails::is_woocommerce_installed() ) {
        return add_query_arg( 'redirect', $redirect, get_permalink( woocommerce_get_page_id('myaccount') ) );
    } else {
...
function fue_save_email( $args ) {
    ...
    if ( $updating ) {
        ...
        FUE_Email_Logger::log_changes( $email_id, $old_email );
    } else {
        do_action('fue_email_created', $email_id, $args);
        FUE_Email_Logger::log( $email_id, __('Created email', 'follow_up_emails') );
    }
...
function fue_get_email_type( $id ) {
    ...
    $type   = ! empty( $terms ) && isset( current( $terms )->name ) ? sanitize_title( current( $terms )->name ) : 'ad-hoc';
...
function fue_exclude_email_address( $email_address, $email_id = 0, $order_id = 0 ) {
    ...
    $count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}followup_email_excludes WHERE email = %s", $email_address) );
