...
class FUE_Admin_Actions {
    ...
    public static function send_manual() {
        ...
        if (! empty($recipients) ) {
            ...
            if ( $email_batch_enabled && count( $recipients ) > $emails_per_batch ) {
                // queue manual emails to schedule them for batched sending
    ...
    static function update_settings() {
        ...
        if ( $section == 'system' ) {
            ...
            $summary_enabled = (empty( $_POST['enable_daily_summary'] ) ) ? 'no' : $_POST['enable_daily_summary'];
            ...
            if ( $action == 'upload' ) {
                ...
                do_action( 'fue_settings_subscribers_imported', $_POST );

                wp_redirect( "admin.php?page=followup-emails-settings&tab=$section&subscribers_added=$added" );
                exit;
            } else {
                $account_label  = $_POST['email_subscriptions_page_title'];
                $account_button = $_POST['email_subscriptions_button_text'];
    ...
    public static function manage_optout() {
        ...
        if ( !empty($post['button_add']) ) {
            ...
            if ( !is_email($email) ) {
                wp_redirect( 'admin.php?page=followup-emails-subscribers&view=opt-outs&error='. urlencode(__('The email address is invalid', 'follow_up_emails') ) );
                exit;
            }
    ...
    public static function optout_delete_email() {
        ...
        if ( !is_email( $email ) ) {
            wp_redirect( 'admin.php?page=followup-emails-subscribers&view=opt-outs&error='. urlencode( __('Invalid email address passed.', 'follow_up_emails') ) );
            exit;
        }
