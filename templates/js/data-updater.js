    ...
    function log_import_data( data ) {
        for ( var x = 0; x < data.length; x++ ) {
            ...
            if ( data[x].status == 'success' ) {
                row = '<p class="success"><span class="dashicons dashicons-yes"></span> '+ entity_label_singular +' #'+ id +' '+ action_label +'</p>';
            } else {
                row = '<p class="failure"><span class="dashicons dashicons-no"></span> '+ entity_label_singular +' #'+ id +' - ' + data[x].reason +'</p>';
            }
